...
QString AddressRegex::ipv6ToString(const quint8* address)
{
    ...
    {
        const quint16 g =
            ((address[(i << 1) + 1]) & 0xFF) | quint16(((address[(i << 1)] & 0xFF) << 8));
        if(!g)
        {
            ++currentZeroCounter;
...
bool AddressRegex::isIpv6(const QString& subject)
{
    static const QRegularExpression re(
        QStringLiteral("((([0-9A-Fa-f]{1,4}:){7}([0-9A-Fa-f]{1,4}|:))|"
...
bool AddressRegex::getMacAddress(const QString& subject, quint8* mac)
{
    if(!mac)
        return false;
    static const QRegularExpression re(QStringLiteral("^([0-9a-fA-F][0-9a-fA-F])(?>:)"
                                                      "([0-9a-fA-F][0-9a-fA-F])(?>:)"
                                                      "([0-9a-fA-F][0-9a-fA-F])(?>:)"
                                                      "([0-9a-fA-F][0-9a-fA-F])(?>:)"
                                                      "([0-9a-fA-F][0-9a-fA-F])(?>:)"
