...
// ─────────────────────────────────────────────────────────────
//
bool VersionRegex::getFullVersion(
    const QString& subject, quint8& major, quint8& minor, quint8& patch, quint32& tag)
{
    static const QRegularExpression re(QStringLiteral("^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\."
...
QString VersionRegex::fullVersionToString(
    const quint8& major, const quint8& minor, const quint8& patch, const quint32& tag)
{
    return QString::number(major) + QStringLiteral(".") + QString::number(minor) +
           QStringLiteral(".") + QString::number(patch) + QStringLiteral(".0x") +
...
{
    return QString::number(major) + QStringLiteral(".") + QString::number(minor) +
           QStringLiteral(".") + QString::number(patch);
}
